...
public class BedwarsOverlay extends Module {
    ...
    public BedwarsOverlay() {
        ...
        this.registerSetting(bombiesMomento = new DescriptionSetting("Dodging moment"));
        overlayHeight = 170;
        overlayWidth = 300;
    ...
    private void drawStats(NetworkPlayerInfo player, FontRenderer fr) {
        ...
        if (Utils.URLS.hypixelApiKey.isEmpty()){
            ...
            fr.drawStringWithShadow(stats[0] + "", statStart.get(StatType.LEVEL), (int)textY, getStarColour(stats[0]));
            fr.drawStringWithShadow(name, statStart.get(StatType.PLAYER_NAME), (int)textY, Colours.WHITE);
            if(stats[1] == 0) {
                fr.drawStringWithShadow("  -", statStart.get(StatType.NICKED), (int)textY, Colours.GREY);
            } else {
                fr.drawStringWithShadow("  +", statStart.get(StatType.NICKED), (int)textY, Colours.RED);
            }
            fr.drawStringWithShadow(stats[2] + "", statStart.get(StatType.WS), (int)textY, getWSColour(stats[2]));

            fr.drawStringWithShadow(bbblr + "", statStart.get(StatType.BBBLR), (int)textY, getBBBLRColour(bbblr));
            fr.drawStringWithShadow(fkdr + "", statStart.get(StatType.FKDR), (int)textY, getFKDRColour(fkdr));
            fr.drawStringWithShadow(wlr + "", statStart.get(StatType.WLR), (int)textY, getWLRColour(wlr));
            fr.drawStringWithShadow(stats[6] + "", statStart.get(StatType.FINALS), (int)textY, getFinalColour(stats[6]));
            fr.drawStringWithShadow(stats[7] + "", statStart.get(StatType.WINS), (int)textY, getFinalColour(stats[7]));
    ...
    private boolean drawError(ScaledResolution sr, FontRenderer fr) {
        ...
        if(Utils.URLS.hypixelApiKey.isEmpty()){
            fr.drawStringWithShadow(noApiKey, (int)(overlayWidth + overlayX.getInput() - overlayWidth/2 - fr.getStringWidth(noApiKey)/2), (int)textY, errorColour);
            textY += fr.FONT_HEIGHT + marginTextY.getInput();
            return true;
        } else if(!Utils.Client.othersExist()){
            fr.drawStringWithShadow(noPlayers, (int)(overlayWidth + overlayX.getInput() - overlayWidth/2 - fr.getStringWidth(noPlayers)/2), (int)textY, errorColour);
            textY += fr.FONT_HEIGHT + marginTextY.getInput();
            return true;
        }
    ...
    private void drawMain(ScaledResolution sr, FontRenderer fr) {
        ...
        for(StatType statType : StatType.values()) {
            fr.drawStringWithShadow(statType + "", (int)textX, (int)textY, mainTextColour);
